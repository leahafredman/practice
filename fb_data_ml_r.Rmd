  ...
  function(df){
    ggplot(df,
           aes(
             ...
             axis2 = age,
             axis3 = gender) ) +
      scale_x_discrete(limits = c('interest', 'age', 'gender')) +
...
library(viridis)
roas_p <-
  ggplot(data = (fb_df_core_roas %>% mutate(gender = as.character(gender)))) +
  geom_point(
    aes(
      x = interest,
      y = roas,
      ...
      shape = gender,
      size = age,
      frame = approved_conversion,
      alpha = .05,
      text = paste('Interest: ', interest, '<br>',
        'Age: ', age, '<br>',
        'Gender: ', ifelse(gender == 1, 'Female', 'Male'), '<br>',
        'Spent: $', spent, '<br>',
        'Conversions: ', approved_conversion, '<br>',
        'ROAS: $', roas, sep = '')
        )
    ) +
  labs(title = 'title') +
  ...
  scale_size_discrete(name = "Age",
                      breaks = c("age_range_30_34",
                                 "age_range_35_39",
                                 "age_range_40_44",
                                 "age_range_45_49"),
                      labels = c("30-34",
...
#library(plotly)
plotly::ggplotly(roas_p, tooltip = 'text')
```
...
roas_p <-
ggplot(data = (fb_df_core_roas %>% mutate(gender = as.factor(gender)))) +
  geom_point(
    aes(
      x = spent,
      y = roas,
