...
fb_df %>% group_by(xyz_camp) %>% select(xyz_camp, ad_id) %>% summarize(n = n(), n_distinct = n_distinct(ad_id))
```
...
However, we don't really want to run these tests each variable at a time.
...
In that case we don't have to bother with interest anymore
